...
import { checkRequiredFields, isObjectkeysExists, machineModalInitialState, selectArrayForModal, selectionData } from '../../../constants'
...
const MachineSetting = ({ formikInstace, dataReset, loader, machineInformation }) => {
  ...
  <>
   <View style={[{ flex: 1, backgroundColor: colors.appBackground, rowGap: thirdOfFullHeight },]}>
    <View style={[productStyles.container, productStyles.gap_5]}>
     <Headings heading={"Machine Mode"} />
     <TouchableOpacity onPress={() => manageModalStates("machine_mode_Options", "machine_mode")}>
      <RowLayout text={values?.machine_mode?.values || ""} Icon={<DownArrowBlack />} />
     </TouchableOpacity>

     <Headings heading={"Banner Mode"} />
     <TouchableOpacity onPress={() => manageModalStates("modeOptions", "machine_mode")}>
      <RowLayout text={values?.machine_mode?.values || ""} Icon={<DownArrowBlack />} />
     </TouchableOpacity>

     <Headings heading={"Machine Category Type"} />
     <TouchableOpacity onPress={() => manageModalStates("machineCategoryTypeOptions", "machine_is_single_category")}>
      <RowLayout text={values?.machine_is_single_category?.values || ""} Icon={<DownArrowBlack />} />
     </TouchableOpacity>
    </View>
