...
import { getCustomarReport } from "../../action";
...
export default function EReceiptReport() {
  ...
  const eReceiptReportData = useMemo(() => {
    if (_eReceiptReport) {
      _data.current = _eReceiptReport;
    }
  ...
  const exportReport = (type) => {
    ...
    let foundElements = Array.isArray(eReceiptReportData?.data)
      ? eReceiptReportData?.data.filter((item) => item && selectedMachineIds?.includes(item?.id))
      : eReceiptReportData?.data &&
        Object.values(eReceiptReportData?.data)
          .flat()
      ...
      <div className="w--full">
        <div className="d--flex justify-content--between align-items--center h-min--36">
          <div className="w-max--400 w--full position--relative">
            <div className="font--lg font--900">E Receipts</div>
          </div>
