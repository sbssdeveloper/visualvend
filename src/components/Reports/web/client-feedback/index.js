...
import { REPORT_CLIENT_FEEDBACK_LIST, REPORT_TYPE_CONST } from "../../constant";
import { setCommonMachineFilter } from "../../../../redux/slices/common-filter-slice";
import { useQuery } from "@tanstack/react-query";
import { getClientFeedbackReport } from "../../action";
...
export default function ClientFeedbackReport() {
  ...
  const [selectedType, setSelectedType] = useState({
    label: REPORT_CLIENT_FEEDBACK_LIST[REPORT_CLIENT_FEEDBACK_LIST.length - 1].name,
    value: REPORT_CLIENT_FEEDBACK_LIST[REPORT_CLIENT_FEEDBACK_LIST.length - 1].id,
  });
  ...
  return (
    <div className="w--full d--flex flex--column gap--md reOrderPage  h--full">
      ...
      <div className="w--full">
        <div className="d--flex justify-content--between align-items--center h-min--36">
          <div className="w-max--400 w--full position--relative">
            <div className="font--lg font--900">Feedback Reports</div>
          </div>
          ...
          <div className="d--flex align-items--center justify-content--end gap--sm w--full">
            <SearchSelect
              ...
              {...{
                ...
                options: REPORT_CLIENT_FEEDBACK_LIST?.map((item) => ({
                  label: item?.name,
                  value: item.id,
                })),
              }}
      ...
      <CommonReportTable data={clientFeedbackData} columnsList={columns} handlePageChange={handlePageChange} totalRecords={clientFeedbackData?.pagination?.total} setLength={setLength} length={length} isExpandAll={isExpandAll} selectedMachineIds={selectedMachineIds} setMachineIds={setMachineIds} additionalConditionsArr={[REPORT_TYPE_CONST.MACHINE, REPORT_TYPE_CONST.SUMMARY, REPORT_TYPE_CONST.FEEDBACK_TYPE, REPORT_TYPE_CONST.PRODUCT, REPORT_TYPE_CONST.LOCATION, REPORT_TYPE_CONST.CUSTOMER, REPORT_TYPE_CONST.ALL_FEEDBACK_LIST].includes(selectedType.value)} isFetching={isFetching} />
    </div>
  );
}
