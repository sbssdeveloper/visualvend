...
import { REPORT_STAFF_LIST, REPORT_TYPE_CONST } from "../../constant";
import { setCommonMachineFilter } from "../../../../redux/slices/common-filter-slice";
import { useQuery } from "@tanstack/react-query";
import { getStaffReport } from "../../action";
...
export default function StaffReport() {
  ...
  const [selectedType, setSelectedType] = useState({
    label: REPORT_STAFF_LIST[0].name,
    value: REPORT_STAFF_LIST[0].id,
  });
  ...
  const clientFeedbackData = useMemo(() => {
    if (_clintFeedback) {
      _data.current = _clintFeedback;
    }
  ...
  const columns = [
    ...
    { key: "employee_full_name", label: "Employee" },
    { key: "product_id", label: "Product ID" },
    { key: "product_name", label: "Product" },
    { key: "product_sku", label: "Product SKU" },
    { key: "machine_name", label: "Machine" },
    { key: "pickup_or_return", label: "Status" },
    { key: "timestamp", label: "Timestamp" },
  ];
  const exportReport = (type) => {
    if (selectedMachineIds.length == 0) return;
    let getDataArray = (array, keys) => array?.map((item) => keys.map((key) => item?.[key] || ""));
    let foundElements = Array.isArray(clientFeedbackData?.data)
      ? clientFeedbackData?.data.filter((item) => item && selectedMachineIds?.includes(item?.id))
      : clientFeedbackData?.data &&
        Object.values(clientFeedbackData?.data)
          .flat()
          .filter((item) => item && selectedMachineIds?.includes(item?.id));

    let dataKeys = ["transaction_id", "job_number", "cost_center", "employee_id", "employee_full_name", "product_id", "product_name", "product_sku", "machine_name", "pickup_or_return", "timestamp"];
  ...
  const handleExcelData = ({ data }) => {
    const sampleData = [["Transaction Id", "Job", "Cost Center", "Emp. ID", "Employee", "Product ID", "Product", "Product SKU", "Machine", "Status", "Timestamp"], ...data];
    exportToExcel(sampleData, "Stock-Level-Report");
  };
  const handlePDFData = ({ data }) => {
    const doc = new jsPDF();
    const transactions = [["Transaction Id", "Job", "Cost Center", "Emp. ID", "Employee", "Product ID", "Product", "Product SKU", "Machine", "Status", "Timestamp"], ...data];
    ...
    <div className="w--full d--flex flex--column gap--md reOrderPage  h--full">
      ...
      <div className="w--full">
        <div className="d--flex justify-content--between align-items--center h-min--36">
          <div className="w-max--400 w--full position--relative">
            <div className="font--lg font--900">Employee Reports</div>
          </div>
          ...
          <div className="d--flex align-items--center justify-content--end gap--sm w--full">
            <SearchSelect
              ...
              {...{
                ...
                options: REPORT_STAFF_LIST.map((item) => ({
                  label: item?.name,
                  value: item.id,
                })),
              }}
      ...
      <div className="w--full  bg--white p--sm radius--md d--flex flex--column gap--sm">
        <div className="w--full h--full d--flex justify-content--between">
          <div className="d--flex gap--sm  w--full w-max--200">
      ...
      <CommonReportTable data={clientFeedbackData} columnsList={columns} handlePageChange={handlePageChange} totalRecords={clientFeedbackData?.pagination?.total} setLength={setLength} length={length} isExpandAll={isExpandAll} selectedMachineIds={selectedMachineIds} setMachineIds={setMachineIds} additionalConditionsArr={[REPORT_TYPE_CONST.MACHINE, REPORT_TYPE_CONST.PRODUCT, REPORT_TYPE_CONST.EMPLOYEE].includes(selectedType.value)} isFetching={isFetching} />
    </div>
  );
}
