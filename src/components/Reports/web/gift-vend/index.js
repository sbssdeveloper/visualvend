...
import { getGiftReport } from "../../action";
...
export default function GiftVendReport() {
  ...
  const giftVendReportData = useMemo(() => {
    if (_giftVendReport) {
      _data.current = _giftVendReport;
    }
  ...
  const columns = [
    ...
    { key: "skin_concern", label: "Skin Concern" },
  ...
  const exportReport = (type) => {
    ...
    let foundElements = Array.isArray(giftVendReportData?.data)
      ? giftVendReportData?.data.filter((item) => item && selectedMachineIds?.includes(item?.id))
      : giftVendReportData?.data &&
        Object.values(giftVendReportData?.data)
          .flat()
          .filter((item) => item && selectedMachineIds?.includes(item?.id));

    let dataKeys = ["transaction_id", "product", "price", "name", "email", "machine_name", "timestamp"];
  ...
  const handleExcelData = ({ data }) => {
    const sampleData = [["Transaction Id", "Product Name", "Product Price", "Name", "Email", "Mobile", "Skin Concern", "Client Name", "Machine Name", "Timestamp"], ...data];
    exportToExcel(sampleData, "Stock-Level-Report");
  };
  const handlePDFData = ({ data }) => {
    const doc = new jsPDF();
    const transactions = [["Transaction Id", "Product Name", "Product Price", "Name", "Email", "Mobile", "Skin Concern", "Client Name", "Machine Name", "Timestamp"], ...data];
      ...
      <div className="w--full">
        <div className="d--flex justify-content--between align-items--center h-min--36">
          <div className="w-max--400 w--full position--relative">
            <div className="font--lg font--900">Gift Reports</div>
          </div>
