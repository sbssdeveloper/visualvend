...
import { getClientFeedbackReport, getMediaAdReport, getMobileMediaAdReport, getRefillReport, getVendErrorReport } from '../action';
...
import { REPORT_CLIENT_FEEDBACK_LIST, REPORT_Media_AD_LIST, REPORT_VEND_ERROR_LIST } from '../constant';
...
const MediaAdvertisement = ({ navigation }) => {
  ...
  const {  data: { data } = {}, isLoading } = useFetchData({ key: "MEDIAAD", fn: getMobileMediaAdReport, start_date: commonDateFilter, end_date: commonEndDate, machine_id: commonMachineIdFilter, type: "frequency",      length: 30 * pageCount, page: 1,
  ...
  return (
    <SafeAreaView>
      ...
      <CurrentMachineHeader showDrawerIcon={false} text={"Advertisement Reports"} />
      ...
      <View style={[appStyles.mainContainer, { paddingHorizontal: undefined }]}>
        ...
        <ProductHeadings heading={"ADVERTISEMENT REPORTS"}
...
<View style={[appStyles.rowSpaceBetweenAlignCenter,{alignSelf:"flex-end"}]}>
                ...
                {/* <ExportWrapper text={"Export All "} Icon={Export} /> */}
                 <ExportWrapper text={"Schedule Advertisment Reports "} Icon={Calender} /> 
            ...
            <View style={[feedbackStyles.feedbackListMainContainer, { marginVertical: 20, padding: 10, }]}>
              ...
              <CollapsibleList filtersBy={selectedValue} querykey={"MEDIAAD"}
                keyOfArrayOrFunctions={"MEDIAADVERTISEMENT"}
                heading={"Advertisment Reports"}
      ...
      <ReportsCategoriesModal
        ...
        arrayData={REPORT_Media_AD_LIST}
      />
