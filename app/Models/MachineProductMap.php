...
class MachineProductMap extends Model
{
    ...
    {
        extract($params);
        $model = self::selectRaw("SUM(IF(product_quantity=0,1,0)) as out_of_stock, SUM(product_max_quantity) as total_quantity,SUM(product_quantity) as remaining_quantity");
        if ($auth->client_id > 0) {
