...
class MachineProductMap extends Model
{
    ...
    {
        extract($params);
        $model = self::selectRaw("product_id, SUM(product_max_quantity) as total_quantity,SUM(product_quantity) as remaining_quantity, (product_max_quantity-product_quantity) as refill, product_name");
        if ($auth->client_id > 0) {
            $model = $model->whereIn("machine_id", $machine_ids)->where("client_id", $auth->client_id);
        }
        $model = $model->get()->first();
