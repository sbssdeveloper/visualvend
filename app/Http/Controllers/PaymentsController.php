...
class PaymentsController extends BaseController
{
    ...
    public function list(Request $request)
    {
        ...
        $modalVendCount .= "SUM(IF(pay_method='pay_to_card' AND transactions.payment_status='SUCCESS',1,0)) as total_card_payment_success, SUM(IF(pay_method IN ('google_pay','paypal','apple_pay','after_pay') AND transactions.payment_status='SUCCESS',1,0)) as total_mobile_payment_success,";
        ...
        $modalVendCount .= "SUM(IF(pay_method='pay_to_card' AND remote_vend_log.status IN('3','4','5','6','7','8','00'),1,0)) as total_card_vend_fail, SUM(IF(pay_method IN ('google_pay','paypal','apple_pay','after_pay') AND remote_vend_log.status IN('3','4','5','6','7','8','00'),1,0)) as total_mobile_vend_fail,";

        $modalVendCount .= "SUM(IF(pay_method='pay_to_card' AND transactions.payment_status='FAILED',1,0)) as total_card_payment_fail, SUM(IF(pay_method IN ('google_pay','paypal','apple_pay','after_pay') AND transactions.payment_status='FAILED',1,0)) as total_mobile_payment_fail,";
    ...
    public function activities(Request $request)
    {
        ...
        $model      = Transaction::selectRaw("IF(pay_method='pay at machine','pay_at_machine',pay_method) as pay_method,remote_vend_log.aisle_number, transactions.amount, transactions.payment_status, machine_id,machine_name,product_id,product_name,transactions.created_at, transactions.response, CASE WHEN response LIKE '%VISA%' THEN 'VISA' WHEN response LIKE '%MASTERCARD%' THEN 'MASTERCARD' WHEN response LIKE '%AMEX%' THEN 'AMEX' ELSE NULL END as card_type, error_log, client_name, remote_vend_log.status as vend_status, failure_reason")->leftJoin('remote_vend_log', 'remote_vend_log.vend_id', '=', 'transactions.vend_uuid');
