...
class DashboardController extends BaseController
{
    ...
    function machine_info($params)
    {
        ...
        $machine_status = MachineHeartBeat::selectRaw('machine_id, last_action, last_sync_time , IF(TIME_TO_SEC(TIMEDIFF(now(),last_sync_time))<=1800,1,0) as connected, IF(TIME_TO_SEC(TIMEDIFF(now(),last_sync_time))>1800 && TIME_TO_SEC(TIMEDIFF(now(),last_sync_time))<=4800,1,0) as fluctuating, IF(TIME_TO_SEC(TIMEDIFF(now(),last_sync_time))>4800,1,0) as offline')->whereIn('machine_id', $machine_ids)->get()->toArray();
        dd($machine_status);
